...
const Footer = () => {
  ...
  const socialLinks: { icon: IconType; color: string; path: string }[] = [
    ...
    { icon: IoShareSocialOutline, color: "hover:bg-cyan-500", path: "#" },
    { icon: IoMailOutline, color: "hover:bg-cyan-500", path: "mailto:contact@danangtrip.com" },
  ];

  const quickLinks = [
    { name: t("footer.about_us"), path: ROUTES.DESTINATIONS },
    { name: t("footer.terms"), path: ROUTES.TOURS },
    { name: t("footer.privacy"), path: ROUTES.DINING },
    { name: t("footer.contact"), path: ROUTES.BLOG },
  ];

  const partnerLinks = [
    { name: t("footer.partner_register"), path: ROUTES.TERMS },
    { name: t("footer.advertise"), path: ROUTES.PRIVACY },
    { name: t("footer.support"), path: ROUTES.HELP },
    { name: t("footer.news"), path: ROUTES.CONTACT },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          ...
          <div className="space-y-6 text-center md:text-left">
            <Link href={ROUTES.HOME} className="flex items-center justify-center md:justify-start gap-2 group">
              ...
              <span className="text-24 font-bold tracking-tight uppercase">
                Đà Nẵng <span className="text-cyan-500 text-24">Trip</span>
              </span>
            </Link>
              ...
              {socialLinks.map((social, idx) => {
                ...
                return (
                  <a
                    key={idx}
                    href={social.path}
                    className={`w-10 h-10 rounded-full bg-gray-800/50 border border-white/5 flex items-center justify-center text-xl transition-all duration-300 hover:scale-110 active:scale-95 ${social.color} hover:text-white text-gray-400`}
                  >
          ...
          {/* Newsletter / App Links placeholder */}
          <div className="text-center md:text-left">
            <h4 className="text-lg font-bold mb-6 text-white uppercase tracking-wider">
              {t("footer.newsletter")}
            </h4>
            ...
            >
              <input
                ...
                required
                placeholder="Email..."
                className="w-full bg-gray-800/50 border border-white/5 rounded-xl py-4 pl-4 pr-12 text-sm focus:ring-2 focus:ring-cyan-500/50 transition-all placeholder-gray-600 h-14 text-white"
              />
              <button
                type="submit"
